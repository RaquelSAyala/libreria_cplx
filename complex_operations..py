
import math

def suma_cplx(c1, c2):
    #Realiza la suma entre dos numeros complejos.
    parteR = c1[0] + c2[0]
    parteI = c1[1] + c2[1]
    return (parteR, parteI)

def resta_cplx(c1, c2):
    #resta de dos numeros complejos.
    parteR = c1[0] - c2[0]
    parteI = c1[1] - c2[1]
    return (parteR, parteI)

def mult_cplx(c1, c2):
    #Realiza la multiplicacion entre dos numeros complejos.
    parteR = c1[0] * c2[0] - c1[1] * c2[1]
    parteI = c1[0] * c2[1] + c1[1] * c2[0]
    return (parteR, parteI)

def div_cplx(c1, c2):
    #Realiza la divicion entre dos numeros complejos.
    denom = c2[0]**2 + c2[1]**2
    c2_conjugate = (c2[0], -c2[1])
    num = mult_cplx(c1, c2_conjugate)
    parteR = num[0] / denom
    parteI = num[1] / denom
    return (parteR, parteI)

def modulo_cplx(c):
    #Calcula el módulo de un número complejo.
    return math.sqrt(c[0]**2 + c[1]**2)

def conjugado_cplx(c):
    #Calcula el conjugado de un número complejo.
    return (c[0], -c[1])

def polar_a_cart(polar):
    #Convierte de coordenadas polares a cartesianas de numeros complejos.   
    r, theta = polar
    parteR = r * math.cos(theta)
    parteI = r * math.sin(theta)
    return (parteR, parteI)

def cart_a_polar(cartesian):
    #Convierte de coordenadas cartesianas a polarea de numeros complejos.
    parteR, parteI = cartesian
    r = modulo_cplx(cartesian)
    theta = math.atan2(parteI, parteR)
    return (r, theta)

def fase_cplx(c):
    #Calcula la fase de un número complejo.
    return math.atan2(c[1], c[0])

